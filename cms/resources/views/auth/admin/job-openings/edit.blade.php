...
<div class="dashboard-wrapper">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fa-solid fa-edit me-2"></i>Edit Job Opening</h2>
                    ...
                    <form action="{{ route('admin.job-openings.update', $jobOpening) }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('job_title') is-invalid @enderror" 
                                    ...
                                    @foreach($jobTitles as $title)
                                        <option value="{{ $title }}" {{ (old('job_title') ?? $jobOpening->job_title) === $title ? 'selected' : '' }}>
                                            {{ $title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('shift') is-invalid @enderror" 
                                    ...
                                    <option value="Day" {{ (old('shift') ?? $jobOpening->shift) === 'Day' ? 'selected' : '' }}>Day</option>
                                    <option value="Night" {{ (old('shift') ?? $jobOpening->shift) === 'Night' ? 'selected' : '' }}>Night</option>
                                </select>
                                @error('shift')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" step="0.01" min="0" 
                                       ...
                                       id="salary" name="salary" value="{{ old('salary') ?? $jobOpening->salary }}" required>
                                @error('salary')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('job_timing') is-invalid @enderror" 
                                       id="job_timing" name="job_timing" value="{{ old('job_timing') ?? $jobOpening->job_timing }}" 
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" min="1" 
                                   ...
                                   value="{{ old('estimated_time_to_hire') ?? $jobOpening->estimated_time_to_hire }}" required>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea class="form-control @error('job_description') is-invalid @enderror" 
                                      id="job_description" name="job_description" rows="5" required>{{ old('job_description') ?? $jobOpening->job_description }}</textarea>
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="fa-solid fa-save me-2"></i>Update Job Opening
                            </button>
